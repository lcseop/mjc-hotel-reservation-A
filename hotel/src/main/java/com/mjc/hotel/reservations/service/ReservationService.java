...
import static com.mjc.hotel.coupon.entity.QCouponIssue.couponIssue;
...
public class ReservationService {
    ...
    public ReservationResponseDto createReservation(ReservationRequestDto requestDto) {
        Member member = memberRepository.findById(requestDto.getMemberId())
                .orElseThrow(() -> new IllegalArgumentException("회원을 찾을 수 없습니다. ID: " + requestDto.getSid()));

        Room room = roomRepository.findById(requestDto.getSid())
        .orElseThrow(() -> new IllegalArgumentException("객실을 찾을 수 없습니다. ID: " + requestDto.getSid()));

//        CouponIssue couponIssue = null;
//        if(requestDto.getSid() != null) {
//            couponIssue = couponIssueRepository.findBy(requestDto.getSid())
//                    .orElseThrow(() -> new IllegalArgumentException("쿠폰을 찾을 수 없습니다. ID: " + requestDto.getSid()));
//        }
...
//                .couponIssue(couponIssue)
    ...
    public void cancelReservation(ReservationCancelDto cancelDto) {
        ...
        List<PointHistory> useHistories = pointHistoryRepository.findAll().stream()
                .filter(ph -> ph.getReservation().getSid().equals(reservation.getSid())
                        && ph.getPointStatus() == PointStatus.USE)
                .collect(Collectors.toList());
        ...
        List<PointHistory> earnHistories = pointHistoryRepository.findAll().stream()
                .filter(ph -> ph.getReservation().getSid().equals(reservation.getSid())
                        && ph.getPointStatus() == PointStatus.ACCUMULATION)
                .collect(Collectors.toList());
