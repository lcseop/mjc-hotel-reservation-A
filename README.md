...
- 예: GitHub Issue와 Pull Request를 중심으로 기능 단위 작업을 관리했습니다.
- 예: 브랜치를 분리해 개발하고, 코드 리뷰 후 `main` 브랜치에 병합했습니다.
- 예: API 명세는 Swagger UI와 Postman 컬렉션으로 공유했습니다.
...
- Flyway 또는 Liquibase를 도입해 운영 DB 스키마 마이그레이션 자동화
- 예약 가능 객실 조회의 동시성 제어 강화
- 프런트엔드 프레임워크 전환 및 E2E 테스트 확대
- CI/CD 파이프라인에서 테스트·정적 분석·컨테이너 이미지 배포 자동화
